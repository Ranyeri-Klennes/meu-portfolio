...
const ThemeToggle = ({ isDark, onToggle }: { isDark: boolean; onToggle: () => void }) => {
  return (
    <button
      onClick={onToggle}
      className={`
        relative w-14 h-7 sm:w-16 sm:h-8 rounded-full p-1.5 
        transition-all duration-[1200ms] outline-none border-none
        ${isDark 
    ...
    >
      <div
        className={`
          w-4.5 h-4.5 sm:w-5.5 sm:h-5.5 rounded-full flex items-center justify-center
          transition-all duration-[1200ms] cubic-bezier(0.4, 0, 0.2, 1) transform
          ${isDark 
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const heroHeight = window.innerHeight * 0.8; // Altura aproximada do hero
      const lastScrollY = lastScrollYRef.current;
      
      // Lógica de visibilidade do navbar
      if (currentScrollY > heroHeight) {
        if (currentScrollY > lastScrollY) {
          // Rolar para baixo -> Ocultar
          setShowNavbar(false);
        } else if (lastScrollY - currentScrollY > 50) {
          // Rolar para cima com delta de 50px (simulando 2 "scrolls") -> Mostrar
          setShowNavbar(true);
        }
      } else {
  ...
  return (
    <div className="min-h-screen font-body bg-gradient-to-b from-[#f0f4ff] via-[#f8fafc] to-[#f0f4ff] dark:from-[#0f172a] dark:via-[#1e293b] dark:to-[#0f172a] relative antialiased transition-colors duration-[1200ms]">
      ...
      {/* ── TOP HOVER TRIGGER (to show navbar) ── */}
      <div 
      ...
      <nav className={`fixed top-4 sm:top-6 left-0 right-0 z-50 px-4 sm:px-6 transition-all duration-300 transform ${showNavbar ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'}`}>
        <div className="max-w-5xl mx-auto glass-premium rounded-[2rem] px-5 sm:px-8 py-3 sm:py-4 flex justify-between items-center relative">
          ...
          <button
            ...
            className="md:hidden flex items-center justify-center p-2 text-slate-600 dark:text-slate-400 hover:bg-slate-100/50 dark:hover:bg-slate-800/50 rounded-xl transition ml-2"
          >
        ...
        <div className="w-full grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="relative w-full max-w-md mx-auto lg:mx-0 aspect-[4/6.3] lg:aspect-[4/6.3] rounded-[2.5rem] overflow-hidden shadow-2xl shadow-blue-900/10 dark:shadow-blue-900/30 float-anim">
            ...
            <img
              ...
              alt="Ranyeri Klennes"
              className={`w-full h-full object-cover object-top transition-opacity duration-[1500ms] ${isDark ? 'opacity-0' : 'opacity-100'}`}
              loading="eager"
            />
            {/* Foto Dark Mode (Sobreposta com Cross-Fade) */}
            <img
              src="/minha-foto-2.jpeg"
              alt="Ranyeri Klennes"
              className={`absolute inset-0 w-full h-full object-cover object-[center_25%] transition-opacity duration-[1500ms] ${isDark ? 'opacity-100' : 'opacity-0'}`}
              loading="eager"
            />
          ...
          <div className="bento-card glass-premium rounded-[2rem] p-7 sm:p-10 flex flex-col justify-between relative overflow-hidden sm:col-span-2 sm:row-span-2">
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-slate-400 leading-relaxed font-medium text-sm sm:text-base">
                Com 3 anos de experiência em engenharia de software e foco em soluções baseadas em dados. Graduado em ADS e pós-graduado em Data Science.
              </p>
            </div>
          ...
          <div className="flex w-max animate-scroll py-6 sm:py-8">
            {[1, 2, 3].map((key) => (
              <React.Fragment key={key}>
                {[
                  { t: "Automatizando-Job", i: <BotIcon className="w-12 h-12 sm:w-14 sm:h-14" />, l: "Python", c: "bg-green-100/80 dark:bg-green-900/30", tx: "text-green-700 dark:text-green-400" },
                  { t: "SmartSchool", i: <SchoolIcon className="w-12 h-12 sm:w-14 sm:h-14" />, l: "C# / .NET", c: "bg-indigo-100/80 dark:bg-indigo-900/30", tx: "text-indigo-700 dark:text-indigo-400" },
                  { t: "Migles", i: <UsersIcon className="w-12 h-12 sm:w-14 sm:h-14" />, l: "Dart", c: "bg-blue-100/80 dark:bg-blue-900/30", tx: "text-blue-700 dark:text-blue-400" }
                ].map((p, idx) => (
                  <div key={idx} className="w-[280px] sm:w-[320px] h-[320px] sm:h-[350px] mx-3 sm:mx-4 group relative glass-card dark:bg-slate-800/80 dark:border-slate-700/50 rounded-[24px] sm:rounded-[28px] overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105 hover:z-20 shrink-0 flex flex-col">
                    <div className="h-44 sm:h-56 bg-gradient-to-br from-slate-100/60 dark:from-slate-700/40 to-slate-200/30 dark:to-slate-800/30 flex items-center justify-center overflow-hidden relative border-b border-white/50 dark:border-slate-700/50">
                      <div className="text-slate-300 dark:text-slate-600 group-hover:scale-125 group-hover:text-blue-500 transition duration-500">{p.i}</div>
                    </div>
                    <div className="p-4 flex flex-col flex-1">
                      <div className="flex justify-between items-center mb-2">
                        <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white truncate">{p.t}</h3>
                        <span className={`text-[10px] ${p.c} backdrop-blur-md ${p.tx} px-2 py-1 rounded-full font-bold shrink-0`}>{p.l}</span>
                      </div>
                      <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 line-clamp-2 leading-relaxed">Repositório dedicado à inovação e otimização técnica.</p>
                      <div className="flex justify-end items-center mt-auto">
                        <a href="#" className="p-2 rounded-xl bg-white/60 dark:bg-slate-700/60 hover:bg-white dark:hover:bg-slate-700 transition shadow-sm border border-white/80 dark:border-slate-600">
